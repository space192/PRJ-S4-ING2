...
Graphe::Graphe(Graphe *c, std::map<std::string, int> capacite) //constructeur par copie créee pour permettre l'optimisation des flots
{
    ...
    m_taille = c->m_taille;
    for(int i = 0 ; i < c->m_tab_FF.size();i++) //on recreer tous les sommets
    {
        m_tab_FF.push_back(new Sommet(c->m_tab_FF[i]->getNum(), c->m_tab_FF[i]->getPoids()));
    }
    for(int i = 0 ; i < c->m_tab_FF.size();i++)
    {
        for(int j = 0 ; j < c->m_tab_FF[i]->getSize();j++) //on copie tous les adjacents afin de faire tourner l'algorithme de Ford Fulkerson sur ce tableau copié
        {
